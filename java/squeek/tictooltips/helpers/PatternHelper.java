...
public class PatternHelper {
	...
	public static List<String> getValidCustomMaterialsOfType(
			Class<? extends CustomMaterial> matClass) {
		List<String> validMats = new ArrayList<String>();
		for (CustomMaterial customMat : TConstructRegistry.customMaterials) {
			customMat = TConstructRegistry.getCustomMaterial(
					customMat.materialID, matClass);
			if (customMat != null
					&& !validMats.contains(customMat.input.getDisplayName())) {
